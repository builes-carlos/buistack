...
Generate vertical slices in natural implementation order (foundations first, features on top — the issue number captures the order). Publish each as a GitHub issue with `ready-for-agent` label, using the same `## What to build / ## Acceptance criteria / ## Part of epic` body format as other issues. No `## Blocked by`, no REST API dependency registration.

If `<last-ship-tag>` is set and is not 'none' (i.e., at least one actual ship tag exists), add a note at the top of each issue body: `> Post-ship addition — not in prod as of <last-ship-tag>.`

Update `CONTEXT.md` if the new area introduces domain terms, architectural components, or constraints not already documented.

## Step 3b — Status revert (pre-ship only)

Before committing, check whether this revision needs to revert the phase Status:

1. Read `CONTEXT.md ## Phases`. Find the current phase row.
2. If its Status is **not** `Complete`: skip this step (already In Progress, no revert needed).
3. If Status is `Complete`: check if a ship tag exists for this phase:
   ```bash
   PHASE_NUM=$(grep -E "^\| [0-9]+ \| .* \| Complete \|" CONTEXT.md | awk -F'|' '{print $2}' | tr -d ' ' | head -1)
   git tag --list "ship/phase-${PHASE_NUM}"
   ```
4. If the tag exists (phase is **Shipped**): do NOT revert Status. The new issues are post-ship additions and remain marked as such.
5. If no tag (phase is **Complete pre-ship**) AND this revision added new issues: revert Status to `In Progress` in `CONTEXT.md ## Phases`. The phase reopens for the new work.
