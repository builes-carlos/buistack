...
1. **Read `vault/00-inbox/MY-INTEGRATIONS.md`** for active data sources and publishing targets
2. **Read `vault/00-inbox/MY-PROFILE.md`** for active projects
3. **Get current timestamp:** Run `date '+%Y-%m-%d %H:%M'` using Bash
4. **Scan existing knowledge base:** Glob `vault/05-knowledge/**/*.md` to understand current state
...
1. Check vault for recent release notes:
   Glob: vault/04-projects/*/release-notes/*.md
...
1. Read all files in vault/05-knowledge/:
   Glob: vault/05-knowledge/**/*.md
...
1. vault/05-knowledge/[file].md — [what needs changing]
2. vault/05-knowledge/[file].md — [what needs changing]

DEPRECATION candidates:
1. vault/05-knowledge/[file].md — [reason: feature removed in v[X]]

STALE entries (>90 days, may need review):
1. vault/05-knowledge/[file].md — last updated [date]
...
Save to: `vault/05-knowledge/[domain]/[topic-slug].md`

```bash
mkdir -p "vault/05-knowledge/[domain]"
```
...
Save to: `vault/05-knowledge/_logs/kb-update-YYYY-MM-DD.md`

```bash
mkdir -p "vault/05-knowledge/_logs"
```
...
Recommended `vault/05-knowledge/` structure:
