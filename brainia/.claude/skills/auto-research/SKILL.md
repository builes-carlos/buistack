...
1. Read relevant files from the vault for existing context:
   - `vault/05-knowledge/` for existing frameworks and mental models
   - `vault/04-projects/` for project-specific context if relevant
...
1. Save the full analysis to `vault/05-knowledge/research/YYYY-MM-DD-[slug].md`
2. If the analysis is long (>3000 words), also create a brief 1-page summary at `vault/05-knowledge/research/YYYY-MM-DD-[slug]-summary.md`
...
- The skill can be run multiple times on related questions — reference previous research files from `vault/05-knowledge/research/`

## Fallback Behavior

This skill requires WebSearch and WebFetch tools. If these are unavailable:
- Fall back to vault-only analysis using existing `vault/05-knowledge/` content
