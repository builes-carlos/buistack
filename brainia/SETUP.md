...
**Step 1: Clone it as the `Brain/` of your life container**
...
3. Select the `AI-Coached-Life/Brain` folder
...
Brainia generates tasks with [Obsidian Tasks emoji format](https://publish.obsidian.md/tasks/Reference/Task+Formats/Tasks+Emoji+Format) (`📅 YYYY-MM-DD`). To use task dashboards and date-based queries:
1. Open Obsidian Settings → Community plugins
2. Browse and install "Tasks" plugin
3. Enable the plugin
4. Brainia tasks will now appear in your Tasks queries (due today, due this week, etc.)

## First Use

### Day 1: Personalize Brainia
...
Brainia will generate personalized news based on your interests.
...
Your second brain is the `Brain/` of an `AI-Coached-Life` container. `Brain/` is the only required folder besides the container root. Every life front is a physical sibling folder of `Brain/`; which ones exist varies per person:
...
If you use an AI assistant with layered instruction files (e.g. Claude Code's `CLAUDE.md`), keep each kind of rule in exactly one place. COG only owns the `Brain/CLAUDE.md` layer:

| Layer | File | Holds |
|-------|------|-------|
| Personal | `~/.claude/CLAUDE.md` (global) | Rules about *you*, true in every project: authorization, commit/push policy, communication style, security defaults. |
...
```
AI-Coached-Life/Brain/         # the second brain folder
...
│   ├── briefs/
│   └── checkins/
├── vault/02-personal/               # Personal domain (private)
│   └── braindumps/
├── vault/03-professional/           # Professional domain
│   └── braindumps/
├── vault/04-projects/               # Project-specific (created by onboarding)
├── vault/05-knowledge/              # Consolidated insights
│   ├── consolidated/
│   ├── patterns/
...
```bash
# Move the AI-Coached-Life container to the iCloud Obsidian folder
mv AI-Coached-Life ~/Library/Mobile\ Documents/iCloud~md~obsidian/Documents/
```

**Step 2: Update Obsidian**
- Open Obsidian on Mac
- "Open folder as vault" → Navigate to iCloud location
- Select the `AI-Coached-Life/Brain` folder
...
```bash
cd ~/Library/Mobile\ Documents/iCloud~md~obsidian/Documents/AI-Coached-Life/Brain
code .
```
...
Open `vault/00-inbox/MY-INTERESTS.md` and edit the topics and sources.
...
```bash
mkdir -p vault/04-projects/my-new-project/{braindumps,competitive,content,planning,resources}
```

Then create `vault/04-projects/my-new-project/PROJECT-OVERVIEW.md`:
```markdown
---
...
Update `vault/00-inbox/MY-PROFILE.md` to include the new project in your active projects list.

## Customizing Skills

Want to customize how Brainia works? Skills are defined in multiple formats for different agents.
...
1. Check `.claude/skills/` folder exists in your Brainia folder
2. Verify each skill folder has a `SKILL.md` file
3. Make sure you're running Claude Code from the `Brain/` folder root
...
**Solutions:**
1. Make sure you're in the `Brain/` folder when running Claude Code
...
**Problem:** Files created by Brainia don't appear in Obsidian

**Solutions:**
1. In Obsidian, make sure you opened the `Brain/` folder as a vault
2. Try refreshing Obsidian (Cmd/Ctrl+R)
3. Check Obsidian settings → Files & Links → "Excluded files" doesn't block Brainia files
...
**Solutions:**
Brainia files are just markdown - conflicts are easy to resolve:
...
1. ✅ Use Brainia daily for 2 weeks to build the habit
...
**Welcome to Brainia!** Your agentic second brain is ready to learn and evolve with you—powered by whichever AI agent you choose.
