...
- **`cog-update.sh` awareness of `fronts/`.** The script compares framework files by path and knows nothing about the fronts tree. Only matters when a pull from upstream actually happens.
...
- **Finishing the COG rename.** `COG-VERSION`, `cog-update.sh`, the `update-cog` skill id, and the issue/discussion links still point at upstream. Cosmetic, orthogonal to the architecture.
